...
class Export {
  ...
  // 安全下载文件，防止浏览器拦截
  async safeDownloadFile(data, filename) {
    try {
      // 检查是否支持文件系统访问API
      if (window.showSaveFilePicker) {
        try {
          ...
          // 定义文件类型映射
          const fileTypeMap = {
            'mm': { 'application/xml': ['.mm'] },
            'emmx': { 'application/json': ['.emmx'] },
            'mmap': { 'application/xml': ['.mmap'] },
            'svg': { 'image/svg+xml': ['.svg'] },
            'png': { 'image/png': ['.png'] },
            'jpg': { 'image/jpeg': ['.jpg'] },
            'pdf': { 'application/pdf': ['.pdf'] },
            'json': { 'application/json': ['.json'] },
            'smm': { 'application/json': ['.smm'] },
            'md': { 'text/markdown': ['.md'] },
            'txt': { 'text/plain': ['.txt'] },
            'xmind': { 'application/zip': ['.xmind'] }
          }

          const accept = fileTypeMap[extension] || { 'application/octet-stream': ['.' + extension] }
          ...
          console.warn('文件保存对话框失败，回退到传统下载:', error)
          // 继续使用传统下载方式
        }
      }
  ...
  async export(type, isDownload = true, name = '思维导图', ...args) {
    if (this[type]) {
      ...
      if (isDownload) {
        try {
          this.safeDownloadFile(result, name + '.' + type)
        } catch (error) {
          throw new Error('浏览器可能拦截了下载，请检查浏览器设置或手动点击下载')
        }
      }
      return result
    } else {
  ...
  // 导出为FreeMind格式
  async freemind(name) {
    if (!this.mindMap.doExportFreeMind) {
      throw new Error('请注册ExportFreeMind插件')
    }
    const res = await this.mindMap.doExportFreeMind.freemind()
    return res
  }

  // 导出为EdrawMind格式
  async edrawmind(name) {
    if (!this.mindMap.doExportEdrawMind) {
      throw new Error('请注册ExportEdrawMind插件')
    }
    const res = await this.mindMap.doExportEdrawMind.edrawmind()
    return res
  }

  // 导出为MindManager格式
  async mindmanager(name) {
    if (!this.mindMap.doExportMindManager) {
      throw new Error('请注册ExportMindManager插件')
    }
    const res = await this.mindMap.doExportMindManager.mindmanager()
    return res
  }
