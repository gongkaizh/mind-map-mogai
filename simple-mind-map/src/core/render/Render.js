...
class Render {
  ...
  // 处理Shift键范围选择
  handleShiftRangeSelection(targetNode) {
    // 如果没有已激活的节点，则直接激活目标节点
    if (this.activeNodeList.length === 0) {
      this.addNodeToActiveList(targetNode)
      this.emitNodeActiveEvent(targetNode)
      return
    }

    // 获取最后一个激活的节点作为起始节点
    const startNode = this.activeNodeList[this.activeNodeList.length - 1]

    // 如果起始节点和目标节点是同一个，直接返回
    if (startNode.uid === targetNode.uid) {
      return
    }

    // 检查两个节点是否是兄弟节点（同一父节点）
    if (startNode.parent && targetNode.parent &&
        startNode.parent.uid === targetNode.parent.uid) {
      // 同级节点范围选择
      this.selectSiblingRange(startNode, targetNode)
    } else {
      // 不同级节点，尝试在同一层级中进行范围选择
      this.selectSameLevelRange(startNode, targetNode)
    }
  }

  // 选择同级节点范围
  selectSiblingRange(startNode, endNode) {
    const parent = startNode.parent
    if (!parent || !parent.children) return

    const siblings = parent.children
    const startIndex = siblings.findIndex(node => node.uid === startNode.uid)
    const endIndex = siblings.findIndex(node => node.uid === endNode.uid)
    ...
    this.emitNodeActiveEvent(endNode)
  }

  // 选择同一层级的节点范围
  selectSameLevelRange(startNode, endNode) {
    // 获取两个节点的层级
    const startLevel = this.getNodeLevel(startNode)
    const endLevel = this.getNodeLevel(endNode)

    // 如果不在同一层级，只激活目标节点
    if (startLevel !== endLevel) {
      this.clearActiveNodeList()
      this.addNodeToActiveList(endNode)
      this.emitNodeActiveEvent(endNode)
      return
    }

    // 获取同一层级的所有节点
    const sameLevelNodes = this.getSameLevelNodes(startLevel)

    const startIndex = sameLevelNodes.findIndex(node => node.uid === startNode.uid)
    const endIndex = sameLevelNodes.findIndex(node => node.uid === endNode.uid)
  ...
  // 获取指定层级的所有节点
  getSameLevelNodes(level) {
    const nodes = []

    const walkNodes = (node, parent, isRoot, layerIndex) => {
      if (layerIndex === level) {
        nodes.push(node)
      }
      if (node.children && node.children.length > 0) {
        node.children.forEach((child, index) => {
          walkNodes(child, node, false, layerIndex + 1)
        })
      }
    }

    walkNodes(this.root, null, true, 0)
    return nodes
  }
  ...
  // 智能检查是否是思维导图数据（即使没有simpleMindMap标识）
  smartCheckMindMapData(data) {
    ...
    if (typeof data === 'string') {
      try {
        ...
        if (this.isMindMapDataStructure(parsedData)) {
          mindMapData = Array.isArray(parsedData) ? parsedData : [parsedData]
        }
      } catch (error) {
        // JSON解析失败，不是思维导图数据
      }
    } else if (typeof data === 'object' && data !== null) {
      if (this.isMindMapDataStructure(data)) {
        mindMapData = Array.isArray(data) ? data : [data]
      }
    }

    return {
      isMindMapData: !!mindMapData,
      data: mindMapData
    }
  }

  // 检查数据结构是否符合思维导图节点格式
  isMindMapDataStructure(data) {
    if (!data || typeof data !== 'object') return false

    // 如果是数组，检查每个元素
    if (Array.isArray(data)) {
      return data.every(item => this.isMindMapNodeStructure(item))
    }

    // 如果是单个对象，检查是否是节点结构
    return this.isMindMapNodeStructure(data)
  }

  // 检查是否是思维导图节点结构
  isMindMapNodeStructure(node) {
    if (!node || typeof node !== 'object') return false

    // 必须有data属性，且data中必须有text属性
    if (!node.data || typeof node.data !== 'object' || typeof node.data.text !== 'string') {
      return false
    }

    // 如果有children属性，必须是数组，且每个子元素都是有效的节点结构
    if (node.children !== undefined) {
      if (!Array.isArray(node.children)) return false
      return node.children.every(child => this.isMindMapNodeStructure(child))
    }
  ...
  copy() {
    ...
    if (!this.mindMap.opt.disabledClipboard) {
      const copyFormat = this.mindMap.opt.copyFormat || 'smm'
      if (copyFormat === 'txt') {
        // 复制为纯文本格式
        let txtContent = ''
        this.beingCopyData.forEach((nodeData, index) => {
          if (index > 0) txtContent += '\n\n'
          txtContent += transformToTxt(nodeData)
        })
        setDataToClipboard(txtContent)
      } else {
        // 默认复制为SMM格式（JSON），添加层级信息以提高XMind兼容性
        const enhancedData = this.enhanceDataForXMindCompatibility(this.beingCopyData)
        setDataToClipboard(createSmmFormatData(enhancedData))
      }
    }
  }
    ...
    this.mindMap.execCommand('CUT_NODE', copyData => {
      ...
      if (!this.mindMap.opt.disabledClipboard) {
        ...
        if (copyFormat === 'txt') {
          // 剪切为纯文本格式
          let txtContent = ''
          copyData.forEach((nodeData, index) => {
            if (index > 0) txtContent += '\n\n'
            txtContent += transformToTxt(nodeData)
          })
          setDataToClipboard(txtContent)
        ...
        if (text) {
          ...
          if (useDefault) {
            ...
            if (checkRes.isSmm) {
              ...
              // 智能检测思维导图数据格式
              const smartCheckRes = this.smartCheckMindMapData(text)
              if (smartCheckRes.isMindMapData) {
                smmData = smartCheckRes.data
              } else {
                text = checkRes.data
              }
            }
          ...
          if (smmData) {
            ...
            // 尝试解析XMind或其他思维导图软件的层级结构
            const hierarchyData = this.parseHierarchicalText(text)
  ...
  copyNode() {
    ...
    // 修复：使用层级感知的排序来保持正确的节点层级关系
    nodeList = this.sortNodeListByHierarchy(nodeList)
    return nodeList.map(node => {
      return copyNodeTree({}, node, true)
    })
  }

  // 新增：按层级关系排序节点列表，确保复制到XMind时层级正确
  ...
  // 新增：获取节点的层级路径
  getNodeHierarchyPath(node) {
    const path = []
    let current = node
    
    // 从当前节点向上遍历到根节点，记录每层的索引
    while (current && current.parent) {
      const indexInParent = current.getIndexInBrothers()
  ...
  // 新增：解析层级文本结构（支持XMind等思维导图软件的复制格式）
  parseHierarchicalText(text) {
    ...
    for (const line of lines) {
      ...
      for (const pattern of hierarchyPatterns) {
        ...
        if (match) {
          ...
          if (pattern.source.includes('\\t')) {
            level = prefix.length // 制表符数量
          } else if (pattern.source.includes('\\s{2,}')) {
            level = Math.floor(prefix.length / 2) // 每2个空格为一级
          } else {
            level = prefix.replace(/\S/g, '').length / 2 // 其他情况按空格计算
          }
          matched = true
          break
        }
      ...
      if (level === 0) {
        ...
        nodeStack[0] = nodeData
      } else if (nodeStack[level - 1]) {
        nodeStack[level - 1].children.push(nodeData)
        nodeStack[level] = nodeData
      } else {
        // 层级跳跃，作为根节点处理
        result.push(nodeData)
        nodeStack[0] = nodeData
