<template>
  ...
  >
    <template v-if="type === 'node'">
      ...
      <div class="splitLine"></div>
      <div class="item" @click="startTraversalFromNode">
        <span class="name">{{ $t('contextmenu.startTraversalFromHere') }}</span>
      </div>
      ...
      <div class="item" @click="aiSuggest" v-if="enableAi">
        <span class="name">{{ $t('contextmenu.aiSuggest') }}</span>
      </div>
      <div class="item" @click="aiExplain" v-if="enableAi">
        <span class="name">{{ $t('contextmenu.aiExplain') }}</span>
      </div>
      <div class="splitLine"></div>
      <div class="item">
    ...
    <template v-if="type === 'svg'">
      ...
      <div class="splitLine"></div>
      <div class="item" @click="startTraversalFromRoot">
        <span class="name">{{ $t('contextmenu.startTraversalFromRoot') }}</span>
      </div>
      <div class="item">
        <span class="name">{{ $t('contextmenu.copyToClipboard') }}</span>
        <span class="el-icon-arrow-right"></span>
...
import { getTextFromHtml, imgToDataUrl, createSmmFormatData } from 'simple-mind-map/src/utils'
...
export default {
  ...
  methods: {
    ...
    async copyToClipboard(type) {
      try {
        ...
        switch (type) {
          case 'smm':
            // 复制选中的节点数据，而不是整个思维导图
            const selectedNodeData = this.mindMap.renderer.copyNode()
            if (selectedNodeData) {
              str = JSON.stringify(createSmmFormatData(selectedNodeData))
            } else {
              // 如果没有选中节点，则复制整个思维导图
              data = this.mindMap.getData(true)
              str = JSON.stringify(createSmmFormatData(data))
            }
            break
          case 'json':
            data = this.mindMap.getData(true)
    ...
    // AI建议
    aiSuggest() {
      this.$bus.$emit('ai_suggest', this.node)
      this.hide()
    },

    // AI解释
    aiExplain() {
      this.$bus.$emit('ai_explain', this.node)
      this.hide()
    },

    // 从当前节点开始主题遍历
    startTraversalFromNode() {
      if (this.mindMap && this.mindMap.topicTraversal) {
        this.mindMap.topicTraversal.startTopicTraversal('depth', this.node)
      }
      this.hide()
    },

    // 从根节点开始主题遍历
    startTraversalFromRoot() {
      if (this.mindMap && this.mindMap.topicTraversal) {
        this.mindMap.topicTraversal.startTopicTraversal('depth', null)
      }
      this.hide()
    }
  }
