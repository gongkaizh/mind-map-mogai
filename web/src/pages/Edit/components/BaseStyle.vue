<template>
  <Sidebar ref="sidebar" :title="$t('baseStyle.title')">
    ...
    >
      ...
      <!-- 紧凑模式 -->
      <div class="title">{{ $t('baseStyle.compactMode') }}</div>
      <div class="row">
        <div class="rowItem">
          <el-switch
...
export default {
  ...
  methods: {
    ...
    // 切换紧凑模式
    toggleCompactMode(val) {
      const spacing = val ? this.compactModeSpacing : this.normalModeSpacing;
      
      // 更新节点内边距
      this.update('paddingX', spacing.paddingX);
      this.update('paddingY', spacing.paddingY);
      
      // 更新二级节点外边距
      if (!this.data.theme.config.second) {
        this.data.theme.config.second = {};
      }
      this.data.theme.config.second.marginX = spacing.marginX;
      this.data.theme.config.second.marginY = spacing.marginY;
      
      // 更新二级以下节点外边距
      if (!this.data.theme.config.node) {
        this.data.theme.config.node = {};
      }
      this.data.theme.config.node.marginX = spacing.marginX;
      this.data.theme.config.node.marginY = spacing.marginY;
      
      // 更新外框内边距
      this.updateOuterFramePadding('outerFramePaddingX', spacing.outerFramePaddingX);
      this.updateOuterFramePadding('outerFramePaddingY', spacing.outerFramePaddingY);
      
      // 更新样式显示
      this.style.marginX = spacing.marginX;
      this.style.marginY = spacing.marginY;
      this.style.paddingX = spacing.paddingX;
      this.style.paddingY = spacing.paddingY;
